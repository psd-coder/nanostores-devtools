...
/**
 * What the panel is told about a store whose value it cannot trust.
 * The words are the view's: the model says what state the slot is in, and a reader needs a
 * sentence. A store is drawn in two places, its own slot in the tree and inside another store's
 * value, and two marks cannot nest, so each caller wraps what it gets back.
 *
 * Only a value that cannot be trusted is marked, so the marker states the consequence and not the
 * mount state.
 */
export function noteFor(store: Store, slot: Exclude<Slot, { state: "live" }>): Note {
  if (slot.state === "never-computed") {
    return { label: "not mounted, never computed", data: {} };
  }

  return { label: "not mounted, may be stale", data: dataForMark(store, slot.value) };
}
